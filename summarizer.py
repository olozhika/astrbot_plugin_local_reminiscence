...
class DailySummarizer:
    def __init__(
        ...
        search_events_func=None,
        prompt_event_summary: str = "",
        prompt_memory_node: str = "",
    ):
    ...
    def _repair_json(self, s: str) -> str:
        ...
        s = re.sub(
            r'(\s*:\s*")([\s\S]*?)("\s*(?:,(?=\s*")|\s*[}\]]))', escape_internal, s
        )

        return s

    async def extract_nodes_from_events(
        self,
        events: List[dict],
        date_str: str,
        ...
        schema_dict = {
            "type": "object",
            "properties": {
                "nodes": {"type": "array", "items": MemoryNode.model_json_schema()},
                "deleted_nodes": {
                    "type": "array",
                    "items": {"type": "string"},
        ...
        schema_str = json.dumps(schema_dict, ensure_ascii=False, indent=2).replace(
...
{self.prompt_memory_node}
    - 今天的日期是 {date_str}。
    - 对于已存在的节点（在已知记忆节点背景中列出），你会在字段中看到它当前的别名 (aliases)。请在已有别名的基础上追加你新发现的别名，不要删除或替换已有的别名。
{context_text}
"""
    ...
    async def generate_summary(
        ...
        schema_dict = {
            "type": "object",
            "properties": {
                "events": {"type": "array", "items": Event.model_json_schema()},
                "daily_reflection": {
                    "type": "string",
...
"""
        ...
        for i, chunk in enumerate(conversation_chunks):
            logger.info(
                f"[APLR] 执行第一阶段总结 (分段 {i + 1}/{len(conversation_chunks)})：提取事件与日感想..."
            )
            ...
            for idx, ev in enumerate(all_events):
                ...
                if isinstance(ev, dict):
                    ...
                    try:
                        ...
                        pass
            logger.debug(
                f"[APLR] 已对今日的 {len(all_events)} 个事件强制进行规范化编号重整（日期: {date_str}）。"
            )
        ...
        if len(all_reflections) == 1:
            ...
            reflections_text = "\n\n".join(
                [
                    f"--- 分段心得 {j + 1} ---\n{r}"
                    for j, r in enumerate(all_reflections)
                ]
            )
        ...
        try:
            ...
            if summary.events:
                ...
                events_list = [
                    e if isinstance(e, dict) else e.model_dump() for e in summary.events
                ]
                nodes_result = await self.extract_nodes_from_events(
                    events_list,
                    date_str,
