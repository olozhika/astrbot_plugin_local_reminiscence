...
class ThematicConsolidator:
    ...
    async def big_consolidation(self, target_min=15, target_max=35):
        ...
        if len(events) < target_min:
            logger.info(
                f"[APLR] 事件数量不足 ({len(events)} < {target_min})，跳过固化。"
            )
            return
        ...
        # 建立 ID 到向量的映射
        id_to_embedding = {
            res["ids"][i]: res["embeddings"][i] for i in range(len(res["ids"]))
        }
        embeddings = np.array([id_to_embedding[eid] for eid in event_ids])
        embeddings = normalize(embeddings)

        # 3. 提取关键词
        keywords_per_doc, word_counts, total_docs, dynamic_stopwords = (
            self._get_dynamic_keywords(pure_narratives)
        )

        # 保存全局高频词供增量固化使用
        self.db.set_consolidation_config(
            "global_high_freq_words",
            json.dumps(list(dynamic_stopwords), ensure_ascii=False),
        )

        # 4. 计算混合距离矩阵
        hybrid_matrix = self._compute_hybrid_distance_matrix(
            embeddings, keywords_per_doc, word_counts, total_docs
        )
        ...
        for threshold in np.linspace(0.3, 0.8, 11):
            ...
            # 检查是否满足基本约束
            is_valid = (target_min <= valid_themes <= target_max) and (
            ...
            if best_clusters is None:
                ...
                if (is_valid and not best_is_valid) or (
                    is_valid == best_is_valid and diff < best_diff
                ):
                    best_threshold = threshold
                    best_clusters = labels
                    best_is_valid = is_valid
        ...
        self.db.set_consolidation_config(
            "last_consolidation_event_count", str(len(events))
        )
        ...
        logger.info(
            f"[APLR] 大固化完成。找到 {len(unique_labels)} 个簇，其中有效主题数: {sum(1 for label in unique_labels if sum(best_clusters == label) >= 4)}"
        )
    ...
    async def incremental_consolidation(self):
        ...
        # 建立 theme_id 到向量的映射
        theme_id_to_centroid = {
            themes_data["ids"][i]: np.array(
                themes_data["embeddings"][i], dtype=np.float32
            )
            for i in range(len(themes_data["ids"]))
        }
        ...
        sql_themes = {
            t["theme_id"]: json.loads(t["keywords"])
            for t in self.db.get_all_thematic_memories()
        }
        ...
        for ev in unthemed_events:
            ...
            if query_res["ids"] and query_res["ids"][0]:
                ...
                clean_text = "".join(
                    re.findall(r"[\u4e00-\u9fa5a-zA-Z0-9]+", ev["narrative"])
                )
                ...
                if dist < threshold:
                    logger.info(
                        f"[APLR] 事件 {eid} 命中主题 {best_theme_id} (距离: {dist:.4f})"
                    )
                    self.db.map_event_to_theme(eid, best_theme_id)
                else:
                    logger.info(
                        f"[APLR] 事件 {eid} 未命中任何主题 (最小距离: {dist:.4f})"
                    )
    ...
    async def summarize_all_themes(self, force=True):
        ...
        for theme in themes:
            ...
            if (
                force
                or (event_count - last_count >= 5)
                or (last_count > 0 and (event_count - last_count) / last_count > 0.2)
            ):
                ...
                if summary:
                    ...
                    # 同步更新向量数据库中的重心
                    try:
                        event_collection = self.vector_db.client.get_collection(
                            "events"
                        )
                        ...
                        if (
                            res.get("embeddings") is not None
                            ...
                            theme_collection = (
                                self.vector_db.client.get_or_create_collection("themes")
                            )
