...
_has_on_agent_done = False
try:
    from astrbot.core.star.register import (
        register_on_agent_done as _register_on_agent_done,
    )
    from astrbot.cli import __version__ as _astrbot_version
    from astrbot.core.utils.version_comparator import VersionComparator

    if VersionComparator.compare_version(_astrbot_version, "4.23.2") >= 0:
        _has_on_agent_done = True
except Exception:
    pass
...
@register(
    "local_reminiscence", "olozhika", "基于定时总结和向量化的本地记忆插件", "1.4.0"
)
class LocalReminiscencePlugin(Star):
    ...
    def __init__(self, context: Context, config: any = None):
        ...
        self.hf_endpoint = self.config.get(
        ...
        try:
            ...
            if self.offline_mode:
                logger.warning(
                    "[APLR] 离线模式已开启，但未检测到 AI 依赖。请先关闭离线模式以安装依赖。"
                )
            else:
                logger.info(
                    "[APLR] 未检测到 AI 依赖，正在后台安装 CPU 版 torch 及相关库 (这可能需要几分钟)..."
                )
                try:
                    # 1. 先安装 torch CPU 版
        ...
        self.target_user_id_list = self.config.get(
            "target_user_id_list", ["Lanya_QQ:FriendMessage:947628188"]
        )
        ...
        embedding_cache_dir_rel = self.config.get(
            "embedding_cache_dir", "APLR_ModelCache"
        )

        embedding_trust_remote_code = to_bool(
            self.config.get("embedding_trust_remote_code", False)
        )
            ...
            for task in asyncio.all_tasks():
                ...
                try:
                    ...
                    if t_name and t_name.startswith("APLR_"):
                        task.cancel()
                        logger.debug(
                            f"[APLR] 检测并通过全局 event loop 取消已存在的后台任务: {t_name}"
                        )
                except Exception:
                    pass
        except Exception as e:
        ...
        while self.__class__._bg_tasks:
            ...
            if not old_task.done():
                old_task.cancel()
                logger.debug(
                    f"[APLR] 已取消热重载前的旧后台任务: {old_task.get_name() if hasattr(old_task, 'get_name') else old_task}"
                )

        t1 = asyncio.create_task(
            self._model_idle_check_loop(), name="APLR_model_idle_check"
        )
        t2 = asyncio.create_task(
            self._auto_summary_check_loop(), name="APLR_auto_summary_check"
        )
                ...
                if auto_summary:
                    ...
                    if now.hour == target_h and now.minute == target_m:
                        ...
                        if self.__class__._last_summary_run_time != current_trigger_key:
                            ...
                            logger.info(
                                f"[APLR] 触发自动每日总结：当前时间符合{boundary_cron} + 1min 时间点 ({target_h:02d}:{target_m:02d})"
                            )
                ...
                if auto_end:
                    ...
                    if now.hour == target_h and now.minute == target_m:
                        ...
                        if self.__class__._last_close_run_time != current_trigger_key:
                            ...
                            logger.info(
                                f"[APLR] 触发自动结束/重置上下文会话：当前时间符合{boundary_cron} + 9min 时间点 ({target_h:02d}:{target_m:02d})"
                            )
    ...
    async def _close_all_active_sessions(self):
        ...
        try:
            ...
            # 使用集合去重
            umos = {
                get_val(conv, "user_id") for conv in convs if get_val(conv, "user_id")
            }
            ...
            logger.info(
                f"[APLR] 自动结束/重置所有旧会话上下文执行完毕。共安全移除了 {closed_count} 个活跃会话的当前指针 (历史数据完好保留在数据库中)。"
            )
        except Exception as e:
            logger.error(
                f"[APLR] 自动结束/重置所有会话上下文时发生异常: {e}", exc_info=True
            )
    ...
    async def _run_automatic_daily_summary(self, target_date: str):
        ...
        try:
            async for result in self._daily_summary_logic(
                event=None, date_str=target_date
            ):
                # 消费生成器
                pass
    ...
    def _is_session_matching(self, unified_id: str) -> bool:
        ...
        if not unified_id:
            return False
        target_list = (
            list(self.target_user_id_list)
            if isinstance(self.target_user_id_list, list)
            else [self.target_user_id_list]
        )
    ...
    def _get_effective_user_ids(self, date_str: str = None) -> List[str]:
        ...
        if len(target_list) == 1 and str(target_list[0]).strip().lower() == "all":
            ...
            if date_str:
                try:
                    for filepath in self.dialog_folder.glob(
                        f"{date_str}_dialog_*.json"
                    ):
    ...
    def _append_to_realtime_log(
        self, unified_id: str, role: str, content: str, event: any = None
    ):
        """将消息追加到实时日志文件 (JSON)"""
        try:
            if event:
                ...
                if is_cron and self.config.get("day_boundary_config", {}).get(
                    "insert_cron_to_chat_history", False
                ):
                    ...
                    self.__class__._active_cron_records[cron_key].append(
                        {
                            "role": "user"
                            if role not in ["assistant", self.ai_name]
                            else "assistant",
                            "content": content
                            if role not in ["assistant", self.ai_name]
                            else f"{role}: {content}",
            ...
            if dialog_file.exists():
                try:
                    ...
                    logger.error(
                        f"[APLR] 读取实时 JSON 失败，可能存在破损 (已备份容错): {e}"
                    )
                    try:
            ...
            logger.error(
                f"[APLR] _append_to_realtime_log 遭遇不可期异常 (不影响聊天进程): {e}"
            )
    ...
    async def on_llm_request(self, event: any, *args, **kwargs):
        ...
        try:
            ...
            actual_event = getattr(event, "event", event)
            is_cron = (
                (actual_event.__class__.__name__ == "CronMessageEvent")
                if event
                else False
            )

            is_realtime_enabled = self.config.get("realtime_recording", False)
            is_cron_history_enabled = self.config.get("day_boundary_config", {}).get(
                "insert_cron_to_chat_history", False
            )
            ...
            if is_realtime_enabled or (is_cron_history_enabled and is_cron):
                if self._is_session_matching(unified_id):
                    ...
                    text = (
                        actual_event.get_plain_text()
                        if hasattr(actual_event, "get_plain_text")
                        else getattr(actual_event, "message_str", "")
                    )
                    if text:
                        self._append_to_realtime_log(
                            unified_id, nickname, text, event=event
                        )
        except Exception as e:
        ...
        req = kwargs.get("req") or (
            args[0] if args and isinstance(args[0], ProviderRequest) else None
        )
        if not req:
            # 尝试从 args 中找
            for arg in args:
        ...
        try:
            ...
            if curr_cid is None:
                ...
                conversation: Conversation = await conv_mgr.get_conversation(
                    uid, curr_cid
                )
                ...
                if (
                    not conversation
                    or not conversation.history
                    or conversation.history == "[]"
                    or history_json_empty
                ):
                    is_new_session = True
            ...
            if is_new_session:
                ...
                if summaries or reflections:
                    ...
                    if reflections:
                        ...
                        other_reflections = [
                            r for r in reflections if r["date"] not in summary_dates
                        ]
                ...
                if tag_match:
                    ...
                    if not current_nickname:
                        ...
                        if nick_match:
                            ...
                            logger.info(
                                f"[APLR] 优先从 system_reminder 标签提取 Nickname 成功: {current_nickname}"
                            )
                    if not current_group_name:
                        ...
                        if group_match:
                            ...
                            logger.info(
                                f"[APLR] 优先从 system_reminder 标签提取 Group Name 成功: {current_group_name}"
                            )
                        ...
                        if sender:
                            for attr in ["nickname", "card", "name"]:
                                ...
                                if val and str(val).strip():
                                    ...
                                    logger.debug(
                                        f"[APLR] 作为备用从 message_obj 提取 Nickname 成功: {current_nickname}"
                                    )
                                    break
                            if not current_nickname and hasattr(sender, "user_id"):
                                current_nickname = str(sender.user_id).strip()
                                logger.debug(
                                    f"[APLR] 作为备用从 user_id 提取 Nickname 成功: {current_nickname}"
                                )
                    ...
                    if not current_group_name:
                        ...
                        if group and hasattr(group, "group_name"):
                            ...
                            if g_name and str(g_name).strip():
                                ...
                                logger.debug(
                                    f"[APLR] 作为备用从 message_obj 提取 Group Name 成功: {current_group_name}"
                                )
                ...
                for text_val in texts_to_search:
                    ...
                    if not current_nickname:
                        ...
                        if nick_match:
                            current_nickname = nick_match.group(1).strip()
                            logger.debug(
                                f"[APLR] 兜底从全文正则提取 Nickname 成功: {current_nickname}"
                            )
                    if not current_group_name:
                        group_match = re.search(
                            r"Group name:\s*([^\n\r,<>]+)", text_val, re.IGNORECASE
                        ...
                        if group_match:
                            current_group_name = group_match.group(1).strip()
                            logger.debug(
                                f"[APLR] 兜底从全文正则提取 Group Name 成功: {current_group_name}"
                            )
            ...
            all_nodes = []
            seen_names = set()
            user_nodes = []  # 初始化为空，确保后续可安全引用

            # 首先尝试获取用户本人节点
            if current_nickname:
                logger.debug(f"[APLR] 正在搜索用户节点: {current_nickname}")
                user_nodes = self.db.search_nodes(
                    current_nickname, limit=1, include_description=False
                )
                    ...
                    try:
                        with self.db._get_conn() as conn:
                            ...
                            if row:
                                user_nodes = [dict(row)]
                                logger.debug(
                                    f"[APLR] 触发逆向匹配，找到用户节点: {user_nodes[0]['name']}"
                                )
                    except Exception as e:
                ...
                if user_nodes:
                    node = user_nodes[0]
                    logger.debug(
                        f"[APLR] 找到用户节点: {node['name']} (类型: {node['type']})"
                    )
            ...
            if current_group_name:
                ...
                group_nodes = self.db.search_nodes(
                    current_group_name, limit=1, include_description=False
                )
                    ...
                    try:
                        with self.db._get_conn() as conn:
                            ...
                            if row:
                                ...
                                logger.debug(
                                    f"[APLR] 触发逆向匹配，找到群聊节点: {group_nodes[0]['name']}"
                                )
                    except Exception as e:
                ...
                if group_nodes:
                    ...
                    if node["name"] not in seen_names:
                        logger.debug(
                            f"[APLR] 找到群聊节点: {node['name']} (类型: {node['type']})"
                        )
                        node["is_group"] = True  # 标记为当前群聊空间
                        all_nodes.append(node)
                        seen_names.add(node["name"])
                        first_level_sources.append(node["name"])
                else:
                    logger.debug(f"[APLR] 未找到群聊节点: {current_group_name}")

            # 将用户节点也加入一级连接源（群聊双源 + 私聊单源）
            if user_nodes:
                src_name = user_nodes[0]["name"]
                if src_name not in first_level_sources:
                    first_level_sources.append(src_name)

            # 上下文感知消歧：从所有源节点的一级连接中匹配消息关键词
            first_level_used = False
            if first_level_sources:
                # 收集所有源的一级连接节点（去重）
                first_level_nodes = []
                seen_fl = set()
                for src_name in first_level_sources:
                    for fl_node in self.db.get_first_level_connected_nodes(src_name):
                        if fl_node["name"] not in seen_fl:
                            seen_fl.add(fl_node["name"])
                            first_level_nodes.append(fl_node)

                if first_level_nodes:
                    msg_keywords = jieba.analyse.extract_tags(message_str, topK=15)
                    first_level_matches = []
                    for fl_node in first_level_nodes:
                        if fl_node["name"] in seen_names:
                            continue
                        # 检查节点名称或别名是否匹配消息关键词
                        node_names = [fl_node["name"]] + json.loads(
                            fl_node.get("aliases") or "[]"
                        )
                        if any(
                            kw in nn.lower() for nn in node_names for kw in msg_keywords
                        ):
                            first_level_matches.append(fl_node)
                    if first_level_matches:
                        logger.debug(
                            f"[APLR] 一级连接消歧命中 {len(first_level_matches)} 个节点 "
                            f"(源: {', '.join(first_level_sources)}): "
                            f"{', '.join(n['name'] for n in first_level_matches)}"
                        )
                        for fl_node in first_level_matches:
                            all_nodes.append(fl_node)
                            seen_names.add(fl_node["name"])
                        first_level_used = True

            # 获取消息相关的节点（若一级连接消歧未命中，回退到全节点搜索）
            if not first_level_used:
                msg_nodes, _ = await self._get_nodes_context(
                    message_str, include_description=False, limit_per_kw=1
                )
                    ...
                    if full_events:
                        ...
                        if remaining and m2 > 0:
                            selected_events.extend(
                                random.sample(remaining, min(len(remaining), m2))
                            )

                        if selected_events:
                            # 强化记忆
                            re_intensity = self.config.get(
    ...
    async def on_llm_response(self, event: any, *args, **kwargs):
        ...
        try:
            ...
            resp = kwargs.get("resp") or (
                args[0] if args else getattr(event, "resp", None)
            )
            if not resp:
                return
            ...
            if hasattr(resp, "reasoning_content") and resp.reasoning_content:
                ...
                think_content = resp.extra.get("think", "") or resp.extra.get(
                    "reasoning", ""
                )
            ...
            if is_cron:
                ...
                if text:
                    ...
                    if text and not text.startswith("I finished this job"):
                        self._append_to_realtime_log(
                            unified_id, ai_name, f"(浅层思考: {text})", event=event
                        )
            else:
                if think_content:
                    self._append_to_realtime_log(
                ...
                if text:
                    ...
                    if text and not text.startswith("I finished this job"):
                        self._append_to_realtime_log(
                            unified_id, ai_name, text, event=event
                        )
        except Exception as e:
            logger.error(f"[APLR] 实时记录 AI 回复异常 (不影响聊天): {e}")

    async def _on_agent_done_handler(
        self, event: any, run_context: any, response: any, *args, **kwargs
    ):
        """当 Agent 运行完成后，如果开启了在 cron job 结束时将完整交互记录写入自带聊天记录，则进行处理"""
        try:
            if not self.config.get("day_boundary_config", {}).get(
                "insert_cron_to_chat_history", False
            ):
                return
            ...
            async def delayed_insert_cron():
                ...
                try:
                    ...
                    if not conv:
                        logger.warning(
                            f"[APLR] 无法获取或创建 cron job 对应的会话: {session_curr_cid}"
                        )
                        return
                    ...
                    if conv.history:
                        try:
                            ...
                            logger.warning(
                                f"[APLR] 无法解析官方会话 {conv.cid} 历史: {exc}"
                            )
                            history = []
                    ...
                    logger.info(
                        f"[APLR] 成功将 {len(records)} 条 Cron Job 完整交互记录延迟并写入 {conv.cid} 官方聊天记录数据库"
                    )
                except Exception as ex:
                    logger.error(
                        f"[APLR] 延迟将 Cron Job 完整交互日志写入自带聊天记录失败: {ex}",
                        exc_info=True,
                    )
            ...
            logger.error(
                f"[APLR] 将 Cron Job 完整交互日志写入自带聊天记录失败: {e}",
                exc_info=True,
            )

    if _has_on_agent_done:
        on_agent_done = _register_on_agent_done()(_on_agent_done_handler)
    ...
    async def on_llm_tool_respond(self, event: any, *args, **kwargs):
        ...
        try:
            ...
            tool_obj = kwargs.get("tool_name") or (
                args[0] if len(args) > 0 else getattr(event, "tool_name", "unknown")
            )
            ...
            tool_args = kwargs.get("args") or (
                args[1] if len(args) > 1 else getattr(event, "args", {})
            )
            result = kwargs.get("result") or (
                args[2] if len(args) > 2 else getattr(event, "result", "")
            )
            ...
            if is_error is False:
                ...
                # 明确为失败
                self._append_to_realtime_log(
                    unified_id, ai_name, "(操作失败: Error)", event=event
                )
            else:
                # 兜底：无明确 is_error 标识时，通过正则做模糊判断，并清洗掉 status / flag 字样
                clean_res_str = res_str
                clean_res_str = re.sub(
                    r"is_?error\s*=\s*\w+", "", clean_res_str, flags=re.IGNORECASE
                )

                error_re = re.compile(
                    r"(error|failed|exception|超时|失败|not found|insufficient|denied)",
                    re.IGNORECASE,
                )
                ...
                if match:
                    ...
                    self._append_to_realtime_log(
                        unified_id, ai_name, f"(操作失败: {keyword})", event=event
                    )
            ...
            if tool_name == "send_message_to_user" and not (is_error is True):
                try:
                    ...
                    target_unified_id = (
                        str(target_session) if target_session else unified_id
                    )
                        ...
                        if isinstance(messages_list, list):
                            for msg in messages_list:
                                ...
                                if type_ == "plain":
                                    ...
                                    msg_parts.append(
                                        f"@{msg.get('mention_user_id', '')}"
                                    )

                        sent_text = "".join(msg_parts).strip()
                        if sent_text and not res_str.strip().startswith("error:"):
                            self._append_to_realtime_log(
                                target_unified_id, ai_name, sent_text, event=event
                            )
                except Exception as sexc:
                    logger.error(
                        f"[APLR] 记录 send_message_to_user 的内容时出错: {sexc}"
                    )
        except Exception as e:
            logger.error(f"[APLR] 实时记录工具活动异常 (不影响聊天): {e}")

    async def _get_nodes_context(
        self,
        text: str,
        ...
        try:
            ...
            for kw in keywords:
                ...
                if is_common_name:
                    ...
                    found_nodes = self.db.search_nodes(
                        kw, limit=limit_per_kw, include_description=include_description
                    )
    ...
    async def _get_nodes_for_summary(
        self, text: str, include_username: bool = False
    ) -> list[dict]:
        """专门为总结/提取环节优化的节点获取逻辑"""
        ...
        def add_special_nodes(current_nodes):
            ...
            important_names = []
            if self.ai_name:
                important_names.append(self.ai_name)
            if include_username and self.username:
                important_names.append(self.username)
        ...
        nodes1_raw, _ = await self._get_nodes_context(
            text, include_description=True, max_nodes=100, limit_per_kw=1
        )
        nodes1_final = add_special_nodes(nodes1_raw)

        # 2. 假如去重后记忆节点数目 < 10 个，再试试检索 2 个
        if len(nodes1_raw) < 10:
            nodes2_raw, _ = await self._get_nodes_context(
                text, include_description=True, max_nodes=100, limit_per_kw=2
            )
    ...
    async def extract_chat_history_command(self, event: AstrMessageEvent):
        ...
        if len(args) < 3:  # /APLR_maintenance extract_history [date]
            yield event.plain_result(
                "用法：/APLR_maintenance extract_history [YYYY-MM-DD]"
            )
            return
    ...
    async def load_model_command(self, event: AstrMessageEvent):
        ...
        yield event.plain_result(
            "⏳ 正在启动/下载并加载向量模型，如果是首次加载可能需要下载模型（耗时几分钟），请稍候..."
        )
        try:
    ...
    async def delete_daily_summary_command(self, event: AstrMessageEvent):
        ...
        if len(args) < 3:
            yield event.plain_result(
                "用法：/APLR_maintenance delete_daily_summary [YYYY-MM-DD]"
            )
            return
        ...
        try:
            deleted_evs, themes_to_up, themes_to_del = self.db.delete_summary_by_date(
                date_str
            )
            if deleted_evs:
            ...
            if themes_to_up:
                ...
                for theme_id in themes_to_up:
                    ...
                    if theme_events:
                        ...
                        try:
                            res = event_collection.get(
                                ids=t_event_ids, include=["embeddings"]
                            )
                            ...
                            if embeddings is not None and len(embeddings) > 0:
                                ...
                                theme_collection.upsert(
                                    ids=[theme_id], embeddings=[centroid.tolist()]
                                )
                        except Exception as theme_vec_err:
                            logger.error(
                                f"[APLR] 重新计算受影响主题 {theme_id} 的重心向量失败: {theme_vec_err}"
                            )
                ...
                for f in self.dialog_folder.iterdir():
                    if f.is_file() and f.name.startswith(date_str):
                        try:
                            ...
                            logger.error(
                                f"[APLR] 删除当日对话文件失败 {f.name}: {file_err}"
                            )
    ...
    async def vectorize_events_command(self, event: AstrMessageEvent):
        ...
        if len(args) < 3:
            yield event.plain_result(
                "用法：/APLR_maintenance vectorize [YYYY-MM-DD] 或 /APLR_maintenance vectorize all"
            )
            return
        ...
        if param == "all":
            yield event.plain_result(
                "⚠️ 正在清空向量库并重新录入所有事件，这可能需要较长时间，请稍候..."
            )
            try:
                # 1. 清空向量库
                ...
                for i in range(0, total, batch_size):
                    ...
                    if i % 100 == 0 or i + batch_size >= total:
                        logger.info(
                            f"[APLR] 事件向量化进度: {min(i + batch_size, total)}/{total}"
                        )
                ...
                if all_themes:
                    try:
                        for t in all_themes:
                            ...
                            t_event_ids = [te["event_id"] for te in theme_events]
                            # 从向量库获取这些事件的向量
                            res = self.vector_db.collection.get(
                                ids=t_event_ids, include=["embeddings"]
                            )
                            embeddings = res.get("embeddings")
                            if embeddings is not None and len(embeddings) > 0:
                        ...
                        if themes_to_add:
                            ...
                            logger.info(
                                f"[APLR] 已重新计算并向量化 {len(themes_to_add)} 个主题。"
                            )
                    except Exception as theme_e:
                        theme_error = str(theme_e)
                        logger.error(f"[APLR] 主题向量化失败: {theme_e}")

                msg = f"✅ 已成功重新向量化全部 {total} 个事件"
                if themes_to_add:
                    msg += f" 和 {len(themes_to_add)} 个主题"
                if theme_error:
                    msg += f"。\n⚠️ 主题向量化过程出现异常（已跳过，不影响事件检索）：{theme_error}"
                yield event.plain_result(msg)
            except Exception as e:
        ...
        try:
            ...
            yield event.plain_result(
                f"✅ 已成功将 {date_str} 的 {len(events)} 个事件向量化。"
            )
        except Exception as e:
            logger.error(f"向量化失败: {e}")
            yield event.plain_result(f"❌ 向量化失败: {e}")

    @aplr_maintenance_group.command("backfill_node_relations")
    async def backfill_node_relations_command(self, event: AstrMessageEvent):
        ...
        try:
            ...
            logger.error(f"全量回填失败: {e}", exc_info=True)
            yield event.plain_result(f"❌ 全量回填失败: {e}")
    ...
    async def update_nodes_command(self, event: AstrMessageEvent):
        ...
        if len(args) < 3:
            yield event.plain_result(
                "用法：/APLR_maintenance update_nodes [YYYY-MM-DD]"
            )
            return
        ...
        yield event.plain_result(
            f"正在从 {date_str} 的 {len(events)} 个事件中提取记忆节点..."
        )
        ...
        try:
            ...
            # 3. 获取基础提示词
            base_system_prompt = (
                self.config.get("default_prompt", "") if self.config else ""
            )
            base_user_prompt = (
                self.config.get("user_prompts", "") if self.config else ""
            )
            ...
            nodes_objs = await self._get_nodes_for_summary(
                search_text, include_username=False
            )
            if nodes_objs:
                logger.debug(
                    f"[APLR] 提取节点环节提供的参考节点: {[n['name'] for n in nodes_objs]}"
                )
            ...
            if nodes_objs:
                ...
                existing_nodes_context = "\n".join([_fmt_node(n) for n in nodes_objs])

            nodes_result = await summarizer.extract_nodes_from_events(
                events,
                date_str,
            ...
            if nodes_result:
                ...
                node_names = [n.name for n in nodes]
                del_msg = (
                    f"，并删除了 {len(deleted_nodes)} 个冗余节点"
                    if deleted_nodes
                    else ""
                )
                yield event.plain_result(
                    f"✅ 已成功更新 {len(nodes)} 个记忆节点：{', '.join(node_names)}{del_msg}"
                )
            else:
                yield event.plain_result(
                    f"⚠️ 未能从 {date_str} 的事件中提取到新的记忆节点。"
                )
        except Exception as e:
    ...
    async def recall_memory_tool(
        ...
        # 检索并展示相关记忆（强化步骤统一在 _get_memory_retrieval_text 内部按强度处理，避免重复强化）
        return await self._get_memory_retrieval_text(
            query, count=count if count > 0 else None
        )
    ...
    async def _get_theme_recall_logic(self, theme_id: str, mode: str = "") -> str:
        ...
        for _ in range(num_to_sample):
            ...
            idx_in_available = random.choices(
                range(len(available_indices)), weights=available_weights, k=1
            )[0]
    ...
    async def recall_recent_events_tool(
        self, event: AstrMessageEvent, days: int = 7, min_score: int = 20
    ) -> str:
    ...
    async def _get_node_events_logic(self, node_name: str) -> str:
        """查看某个记忆节点的关联事件列表"""
        # 精确匹配节点名称
        nodes = self.db.search_nodes(node_name, limit=5)
        matched = [n for n in nodes if n["name"] == node_name]
        if not matched:
            # 尝试别名精确匹配
            all_nodes = self.db.search_nodes(node_name, limit=10)
            for n in all_nodes:
                aliases = json.loads(n.get("aliases") or "[]")
                if node_name in aliases:
                    ...
                    break
            if not matched:
                return f"未找到名为 '{node_name}' 的记忆节点。"

        node = matched[0]
        related_ids = json.loads(node.get("related_event_ids") or "[]")
        if not related_ids:
            return f"节点 '{node_name}' 暂无关联事件。"
        ...
        res = f"节点 '{node_name}' 的关联事件（共 {len(events)} 条，展示 {len(selected)} 条）：\n"
        for ev in selected:
    ...
    async def deep_recall_command(self, event: AstrMessageEvent):
        """深度回想。用法：/APLR_recall deep [事件ID|主题ID|日期|节点名称] [模式(可选)]。"""
        msg = event.message_str.strip()
        # 移除命令名部分
        ...
        if not content:
            yield event.plain_result(
                "用法：/APLR_recall deep [事件ID|主题ID|日期] [模式(可选)]\n模式支持：类人、时间、情绪、随机"
            )
            return
    ...
    async def recall_recent_events_command(
        self, event: AstrMessageEvent, days: str = "7", min_score: str = "20"
    ):
        ...
        try:
            ...
            yield event.plain_result(
                "请输入有效的数字（如：/APLR_recall recent 7 20）。"
            )
            return
    ...
    async def write_node_command(self, event: AstrMessageEvent):
        ...
        if not content:
            yield event.plain_result(
                "用法：/APLR_maintenance write_node [节点名] [类型] [描述]"
            )
            return
    ...
    async def write_node_tool(
        self, event: AstrMessageEvent, name: str, node_type: str, description: str
    ) -> str:
    ...
    async def _get_memory_retrieval_text(self, query: str, count: int = None) -> str:
        ...
        search_results = self.vector_db.search_all(
            search_query, top_n_events=top_n, top_n_themes=5
        )
        ...
        for res in search_results:
            ...
            if rtype == "event":
                ...
                if ev:
                    ...
                    score = self._calculate_single_event_score(
                        ev, relevance, query_terms, now
                    )
                    candidates.append({"type": "event", "score": score, "data": ev})
            elif rtype == "theme":
                ...
                if theme:
                    ...
                    theme_score = self._calculate_theme_score(
                        theme, theme_events, relevance, now
                    )
                    candidates.append(
                        {"type": "theme", "score": theme_score, "data": theme}
                    )
        ...
        for i, tid in enumerate(group_order):
            ...
            for ev in group["events"]:
                displayed_event_ids.append(ev["event_id"])
                resp += f"📜 **【具体往事】** ({ev['date']})\n"
        ...
        # 按配置强度统一强化本次展示的事件（强度为 0 时表示关闭强化）
        re_intensity = self.config.get("reinforcement_intensity", 1.0)
        if re_intensity > 0 and displayed_event_ids:
            self.db.reinforce_memory(displayed_event_ids)
            logger.info(f"[APLR] 强化了 {len(displayed_event_ids)} 条记忆的权重")
    ...
    def _calculate_theme_score(
        self, theme, theme_events, vector_relevance, now_date
    ) -> float:
        """计算主题记忆的综合得分"""
        if not theme_events:
        ...
        # 1. 计算平均重要性和情感强度
        avg_importance = sum(e.get("importance", 5) for e in theme_events) / len(
            theme_events
        )
        avg_intensity = sum(
            e.get("emotional_intensity", 5) for e in theme_events
        ) / len(theme_events)
        ...
        try:
            ...
            representative_date = datetime.strptime(
                representative_date_str, "%Y-%m-%d"
            ).date()
    ...
    def _calculate_single_event_score(
        self, ev, vector_relevance, query_terms, now_date
    ) -> float:
        """计算单个事件的综合记忆得分"""
        try:
            ev_date = datetime.strptime(ev["date"], "%Y-%m-%d").date()
        ...
        score = (importance**w_importance) * (
            intensity**w_intensity
        ) * relevance_score * date_weight * keyword_bonus + reinforcement_boost
        return score
    ...
    def _generate_narrative_bridge(self, clusters: List[List[dict]]) -> str:
        ...
        for i, cluster in enumerate(clusters):
            ...
            for ev in cluster:
                ...
                narrative += (
                    f"  - {ev['narrative']} [ID: {ev['event_id']}]{theme_str}\n"
                )
            narrative += "---\n"
        return narrative
    ...
    def _calculate_coherence(self, target_event: dict, all_events: List[dict]) -> float:
        ...
        try:
            ...
            adjacent_count = 0
            for e in all_events:
                if e["event_id"] == target_event["event_id"]:
                    continue
        ...
        for other_ev in all_events:
            if other_ev["event_id"] != target_event["event_id"]:
                if self.db._has_relation(
                    target_event["event_id"], other_ev["event_id"]
                ):
                    coherence += 0.3
                    break
    ...
    async def _search_events_for_summary(
        self, query: str, top_n: int = 5, exclude_date: str = None
    ) -> List[dict]:
        """为总结过程提供的事件搜索接口"""
        return await self._get_relevant_events(
            query, top_n=top_n, exclude_date=exclude_date
        )

    async def _daily_summary_logic(
        self, event: AstrMessageEvent = None, date_str: str = None
    ):
        ...
        if not has_local_json:
            try:
                ...
                if core_db_path.exists():
                    logger.info(
                        f"[APLR] 未找到日期 {date_str} 的记录文件，正在尝试从核心数据库提取..."
                    )
                    for target_user_id in effective_user_ids:
                        clean_dialogue_with_different_limits(
                            ...
                            target_date=date_str,
                            target_user_id=target_user_id,
                            day_boundary_config=self.config.get(
                                "day_boundary_config", {}
                            ),
                        )
            except Exception as e:
        ...
        for target_user_id in effective_user_ids:
            ...
            try:
                ...
                if conversations:
                    ...
                    # 预估文件大小 (KB)
                    file_text_list = [
                        f"[{m.get('timestamp', '')}] {m.get('role', 'unknown')}: {m.get('content', '')}\n"
                    ...
                    for i, msg in enumerate(conversations):
                        ...
                        if last_time and timestamp:
                            try:
                                curr_t = datetime.strptime(
                                    timestamp, "%Y-%m-%d %H:%M:%S"
                                )
                                prev_t = datetime.strptime(
                        ...
                        if (
                            ...
                            and (current_size + msg_size > max_kb)
                            and time_gap_exceeded
                        ):
                            conversation_chunks.append("".join(current_chunk))
                            current_chunk = [
                            ...
                            current_size = (
                                len(current_chunk[0].encode("utf-8")) / 1024.0
                            )
                ...
                logger.info(
                    f"[APLR] 读取记录 ({target_user_id}) 的时候出了点小状况：{e}"
                )
        ...
        logger.info(
            f"[APLR] 正在总结 {date_str} 的点点滴滴，分成了 {len(conversation_chunks)} 段处理，稍等一下哦..."
        )
        ...
        try:
            ...
            if not provider_id:
                try:
                    default_provider = (
                        self.context.get_using_provider(umo=umo)
                    ...
                    logger.warning(
                        f"[APLR] 无法通过 get_using_provider 获取默认 LLM ID: {e}"
                    )
            ...
            try:
                ...
                if persona and persona.get("prompt"):
                    ...
                    logger.debug(
                        f"[APLR] 已成功注入 Astrbot 自带的人格设定 (Persona: {persona.get('name', 'Unknown')})"
                    )
                    # logger.debug(f"base sys prompt: {base_system_prompt}")
            except Exception as pe:
                logger.warning(f"[APLR] 尝试获取 Astrbot 人格设定失败: {pe}")
            ...
            if 0 <= h < 12:
                ...
                logger.debug(
                    f"[APLR] 检测到跨日时间在中午12点之前的凌晨 ({time_str})，已成功注入熬夜分界线提示词。"
                )
            ...
            known_node_names = {n["name"] for n in nodes} if nodes else set()
            if nodes:
                logger.debug(
                    f"[APLR] 每日总结环节提供的参考节点: {[n['name'] for n in nodes]}"
                )
            ...
            if nodes:
                ...
                existing_nodes_context = "\n".join([_fmt_node(n) for n in nodes])
                ...
                if summary and summary.events:
                    ...
                    if not self.consolidator:
                        ...
                        self.consolidator = ThematicConsolidator(
                            ...
                            prompt_theme_summary=prompts_config.get(
                                "theme_summary", ""
                            ),
                        )
                    else:
                        ...
                        self.consolidator.prompt_theme_summary = prompts_config.get(
                            "theme_summary", ""
                        )
        ...
        try:
            ...
            try:
                logger.info(
                    f"[APLR] 开始检查并自动清除 {date_str} 的旧事件与日总结记录..."
                )
                deleted_evs, themes_to_up, themes_to_del = (
                    self.db.delete_summary_by_date(date_str)
                )
                if deleted_evs:
                    self.vector_db.delete_events(deleted_evs)
                    logger.info(
                        f"[APLR] 已自动清除 {len(deleted_evs)} 条已存在的当日事件向量"
                    )
                if themes_to_del:
                    self.vector_db.delete_themes(themes_to_del)
                ...
                if themes_to_up:
                    ...
                    logger.info(
                        f"[APLR] 已同步更新受影响的 {len(themes_to_up)} 个主题的重心向量"
                    )
            except Exception as clear_err:
                logger.error(
                    f"[APLR] 自动清理 {date_str} 历史事件和总结失败: {clear_err}",
                    exc_info=True,
                )
            ...
            try:
                ...
                logger.info(
                    f"[APLR] 已自动将 {date_str} 的 {len(events_to_vec)} 个事件同步到向量数据库。"
                )
            ...
            try:
                all_events_count = len(self.db.get_all_events())
                last_threshold = self.db.get_consolidation_config("last_threshold")
                should_remind = False

                if last_threshold is None:
                    # 从未执行过大固化
                    if all_events_count >= 300:
                        should_remind = True
                else:
                    # 已执行过大固化，检查是否翻倍
                    last_count_str = self.db.get_consolidation_config(
                        "last_consolidation_event_count"
                    )
                    if last_count_str:
                        last_count = int(last_count_str)
                        if all_events_count >= last_count * 2:
                            should_remind = True

                if should_remind:
                    reminder_msg = (
                        f"💡 你的记忆事件已积累到 {all_events_count} 条，"
                        "建议使用 /memory_consolidation 指令对记忆进行主题聚类，以提高回忆效率。"
                    )
    ...
    async def memory_consolidation_command(self, event: AstrMessageEvent):
        ...
        yield event.plain_result(
            "🚀 正在启动全局记忆主题归类（大固化），这可能需要较长时间，请耐心等待..."
        )
    ...
    async def recall_theme_command(self, event: AstrMessageEvent):
        ...
        if len(args) > 2:  # /APLR_recall theme [id]
            ...
            summary = next(
                (t["summary"] for t in themes if t["theme_id"] == theme_id), "暂无总结"
            )
    ...
    def is_model_ready(self) -> bool:
        ...
        return (
            ...
            and hasattr(self.vector_db, "model")
        )
